...
Future<void> runParkingChecklist() async {
  ...
  await flutterTts
      .awaitSpeakCompletion(true); // أهم سطر: استنى لما تخلص كلامك خالص
  ...
  // البداية
  await flutterTts.speak("Parking check list");
  ...
  for (var step in checklist) {
    ...
    while (!confirmed) {
      ...
      // 2. فاصـل أمان 600 مللي ثانية عشان نضمن إن صوت السماعة مطلعش في المايك
  ...
  // النهاية
  await flutterTts.speak("PARKING CHECK LIST COMPLETED");
}
...
Future<String> transcribeWithGroq(String audioPath, String apiKey) async {
  try {
    ...
    if (response.statusCode == 200) {
      ...
      // القاموس الذكي اللي كنت بتستخدمه
