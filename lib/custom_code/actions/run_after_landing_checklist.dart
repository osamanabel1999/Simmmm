...
Future<void> runAfterLandingChecklist() async {
  ...
  // قائمة المهام لمرحلة AFTER LANDING
  List<Map<String, String>> checklist = [
    {'q': 'RAY-DAR AND P-W-S', 'a': 'off'},
  ];

  // البداية
  await flutterTts.speak("AFTER LANDING CHECK LIST");
  ...
  // النهاية
  await flutterTts.speak("AFTER LANDING CHECK LIST COMPLETED");
}
...
Future<String> transcribeWithGroq(String audioPath, String apiKey) async {
  try {
    ...
    request.fields['prompt'] =
        'Aviation checklist commands: off, set, checked, on, removed, nav, both.';
