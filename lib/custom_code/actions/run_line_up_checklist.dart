...
Future<void> runLineUpChecklist() async {
  ...
  // قائمة المهام لمرحلة LINE UP
  List<Map<String, String>> checklist = [
    {'q': 'TAKE OFF RUNWAY', 'a': 'confirmed|checked'},
    {
      'q': 'TEE-KAS',
      'a': 'ta ra|ta|ra|tara|tera|tere'
    }, // الرد المطلوب TA RA وبنعالجه في القاموس
    {'q': 'PACK 1 and 2', 'a': 'on|off|checked'},
  ];

  // البداية
  await flutterTts.speak("LINE UP CHECK LIST");
  ...
  // النهاية
  await flutterTts.speak("LINE UP CHECK LIST COMPLETED");
}
...
Future<String> transcribeWithGroq(String audioPath, String apiKey) async {
  try {
    ...
    request.fields['prompt'] =
        'Aviation checklist commands: confirmed, checked, ta ra, on, off, set, removed, nav, both.';
    ...
    if (response.statusCode == 200) {
      ...
      // القاموس الذكي المحدث مع معالجة احتمالات TA RA
